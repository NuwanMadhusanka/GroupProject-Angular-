...
export class VehicleMoreDetailsComponent implements OnInit {
 ...
 // vehicleId: any;
  ...
  isUpdate(option) {
    ...
    if ((option === 6)) { this.isUpdateVariable = true; this.selectOption = option; this.placeHolder = "New Document License"; this.updateName = "License"; this.updateVariable = this.vehicleData.document_lic; }
    ...
    if (this.selectOption == 5) {
      ...
      if (this.selectOption == 6) {
        if ((this.updateVariable == "")) {
          ...
          this.vehicleData.document_lic = this.updateVariable;
          this.errorUpdateMessage = "";
          this.isUpdateVariable = false;
          this.confirmUpdate = true;
        }
